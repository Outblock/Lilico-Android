...
class FclAuthzDialog : BottomSheetDialogFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        ioScope {
            ...
            if (isShowing()) {
                uiScope {
                    binding.securityCheckWrapper.setVisible()
                    val titleMap = response.data?.messages?.title?.i18n
                    val descMap = response.data?.messages?.description?.i18n
