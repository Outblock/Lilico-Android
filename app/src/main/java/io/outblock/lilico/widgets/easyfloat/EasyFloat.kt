package io.outblock.lilico.widgets.easyfloat

import android.app.Activity
import android.content.Context
import android.view.View
import android.view.WindowManager
import io.outblock.lilico.widgets.easyfloat.core.FloatingWindowManager
...
import io.outblock.lilico.widgets.easyfloat.permission.PermissionUtils
...
class EasyFloat {
    ...
    companion object {
        ...
        fun isHided(tag: String? = null) = FloatingWindowManager.getHelper(tag)?.isHided() ?: false
        ...
        fun getWindowManager(tag: String? = null): WindowManager? = FloatingWindowManager.getHelper(tag)?.windowManager

        fun getWindowLayoutParam(tag: String? = null): WindowManager.LayoutParams? = FloatingWindowManager.getHelper(tag)?.params

        fun offsetWindow(tag: String? = null, x: Int, y: Int) = FloatingWindowManager.getHelper(tag)?.offsetWindow(x, y)

        fun updateHeight(tag: String? = null, offset: Int) = FloatingWindowManager.getHelper(tag)?.updateHeight(offset)

        fun updateLayout(tag: String? = null, view: View) = FloatingWindowManager.getHelper(tag)?.updateLayout(view)

        fun getWindowHeight(tag: String? = null): Int = FloatingWindowManager.getHelper(tag)?.getWindowHeight() ?: 0
    ...
    class Builder(private val activity: Context) : OnPermissionResult {
        ...
        fun setHardKeyEventEnable(enable: Boolean) = apply { config.hardKeyEventEnable = enable }
         ...
         * 设置浮窗是否处理触摸事件
         */
        fun setTouchEnable(touchEnable: Boolean) = apply { config.isTouchEnable = touchEnable }


        /**
         * 忽略拖拽事件的 View
         */
        fun setIgnoreDragViewList(list: List<Int>) = apply { config.ignoreDragViewList = list }

        /**
         * 拖拽事件发生后，不再分发事件给这些 View, (例如点击事件等不会调用)
         */
        fun setForceDragViewList(list: List<Int>) = apply { config.forceDragViewList = list }
        ...
        fun setWindowHeight(height: Int) = apply { config.windowHeight = height }
